...
appD = DjangoDash('sensorDView')

appD.layout = html.Div([
    dcc.Graph(id = 'sensorDGraph',animate=True),
    dcc.Interval(
        id='graph-D-update',
        interval=1*4000
    )
...
@appD.callback(Output('sensorDGraph','figure'),events=[Event('graph-D-update','interval')])
def senceDUpdaeGraph():
    global X
    global Y
    X.append(X[-1]+1)
    Y.append(db.child('sence4').child('value').get().val())

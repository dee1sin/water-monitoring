...
app = DjangoDash('sensorAView')

app.layout = html.Div([
    dcc.Graph(id = 'sensorAGraph',animate=True),
    dcc.Interval(
        id='graph-A-update',
        interval=1*4000
    )
...
@app.callback(Output('sensorAGraph','figure'),events=[Event('graph-A-update','interval')])
def senceAUpdaeGraph():
    global X
    global Y
    X.append(X[-1]+1)
    Y.append(db.child('sence1').child('value').get().val())
